// Rectified app.js - Unicorn Madagascar (demo live ticks)
document.addEventListener("DOMContentLoaded", () => {
  ...
  const SMA_WINDOW = 20;
  let numb_;
  const volatilitySymbols = ["BOOM1000","CRASH1000","BOOM900","CRASH900","BOOM600","CRASH600","BOOM500","CRASH500","R_100","R_75","R_50","R_25"];
  ...
  function initSymbols(){
    ...
    volatilitySymbols.forEach(sym => {
      ...
      el.id = `symbol-${sym}`;
      el.textContent = sym.replace("_"," ").replace(/([A-Z]+)(\d+)/,"$1 $2");
  ...
  function drawChart(){
    ...
    if(len>0){
      ...
      let pnl=0;
      trades.forEach(tr=>{
        const diff=tr.type==="BUY"?lastPrice-tr.entry:lastPrice-tr.entry;
  ...
  function executeTrade(type){
    ...
    const count = type==="BUY"?parseInt(buynumb.value)||1:parseInt(sellnumb.value)||1;

    for(let i=0;i<count;i++){
      trades.push({ symbol:currentSymbol,type,stake,multiplier,entry:lastPrices[currentSymbol]||0 });
    }
    logHistory(`${type} ${currentSymbol} x${count} executed at ${lastPrices[currentSymbol]||0}`);
    drawChart();
  }
  ...
  closeBtn.onclick=()=>{
    trades=[];
    drawChart();
    logHistory("All trades cleared");
  };

  function updatePnL(){
    // Handled inside drawChart
  }
