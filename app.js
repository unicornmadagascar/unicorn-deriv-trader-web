document.addEventListener("DOMContentLoaded", () => {
  ...
  let smoothedVolatility = 0, smoothedATR = 0, smoothedEMA = 0;
  ...
  function initGauges() {
    ...
    if (!gaugeDashboard) {
      ...
      gaugeDashboard.style.flexDirection = "row"; // horizontal
  ...
  function drawGauge(canvas, value) {
    ...
    // Animated progress
    let currentAngle = canvas.dataset.currentAngle ? parseFloat(canvas.dataset.currentAngle) : 0;
    const targetAngle = (value / 100) * 2 * Math.PI;
    currentAngle += (targetAngle - currentAngle) * 0.1; // transition smooth
    canvas.dataset.currentAngle = currentAngle;
    ctx.beginPath(); ctx.arc(w / 2, h / 2, radius, -Math.PI / 2, -Math.PI / 2 + currentAngle);
  ...
  function handleMessage(data) {
    ...
    if (data.msg_type === "tick" && data.tick?.symbol) {
      ...
      if (symbol === currentSymbol) { chartData.push(price); chartTimes.push(tick.epoch); if (chartData.length > 300) { chartData.shift(); chartTimes.shift(); } drawChart(); }
      const el = document.getElementById(`symbol-${symbol}`);
      if (el) { const span = el.querySelector(".symbolValue"); let direction = "➡"; let color = "#666";
