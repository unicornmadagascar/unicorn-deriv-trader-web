document.addEventListener("DOMContentLoaded", () => {
  ...
  function drawChart() {
    ...
    // Grid & Y-axis labels
    ctx.strokeStyle = "#ddd"; ctx.lineWidth = 0.8; ctx.fillStyle = "#555";
    ...
    ctx.fillStyle = "red"; ctx.beginPath(); ctx.arc(padding + ((len - 1)/(len - 1))*w, yPrice, 5, 0, 2*Math.PI); ctx.fill();
  ...
  function initGauges() {
    ...
    // Style flex to remove gaps
    gaugeContainer.style.display = "flex";
    gaugeContainer.style.justifyContent = "flex-start";
  ...
  function calculateATR() {
    if(chartData.length<2) return 0;
    let sum = 0;
    for(let i=1;i<chartData.length;i++) sum += Math.abs(chartData[i]-chartData[i-1]);
    return Math.min(sum/chartData.length*10,100); // scaled for gauge
  }
  ...
  function calculateEMA(period=10) {
    ...
    let k = 2/(period+1);
    let ema = chartData[0];
    for(let i=1;i<chartData.length;i++) ema = chartData[i]*k + ema*(1-k);
    return Math.min(Math.abs(chartData[chartData.length-1]-ema)*50,100); // scaled
  }
  ...
  buyBtn.onclick=()=>logTrade("BUY"); sellBtn.onclick=()=>logTrade("SELL"); closeBtn.onclick=()=>logTrade("CLOSE");
