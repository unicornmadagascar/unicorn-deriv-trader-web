...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // chart
// --- Fetch contract entry and profit for a trade ---
function fetchContractEntry(trade) {
    if (!authorized || !ws || ws.readyState !== WebSocket.OPEN) return;

    // Request info for this trade
    ws.send(JSON.stringify({
        proposal_open_contract: 1,
        contract_id: trade.contract_id
    }));

    // Listen once for the server response
    const listener = (msg) => {
        ...
        if (data.msg_type === "proposal_open_contract" && data.proposal_open_contract) {
            ...
            // Only update the relevant trade
            if (poc.contract_id === trade.contract_id) {
                trade.entry = poc.entry_spot;
                trade.profit = poc.profit;

                // Redraw chart and P/L after getting data
                drawChart();
                updatePnL();

                // Remove listener after handled
...
// --- Draw chart including trades, entry lines, and P/L ---
function drawChart() {
    ...
    trades.forEach(tr => {
        ...
        const x = padding + ((len - 1) / (len - 1)) * w; // rightmost
